...
public final class CommandExample {
    ...
    public static ICommand create(){
        return new CommandBuilder(NickelAPI.MODID)
                .require(0)
                ...
                .then(literals()
                        .when("nbt").then($NBT("nbt")
                                ...
                                .defaultAs(new NBTTagCompound())
                                .asOptional()
                                .then(execute(ctx -> plain("Your NBT is: ").then(ctx.get("nbt", NBTCompoundNode.class).toString())
                                        .sendTo(ctx.getSender()))))
                        .when("stack").then($itemStack("itemStack")
                                .then(execute(ctx -> {
                                    final ItemStack stack = ctx.get("itemStack", ItemStackNode.class);
                                    translation(stack.getTranslationKey())
