...
import top.qiguaiaaaa.nickel.command.builder.NoSplitNodeBuilder;
import top.qiguaiaaaa.nickel.command.context.ExecuteContext;
import top.qiguaiaaaa.nickel.command.node.functional.ForEachNode;
import top.qiguaiaaaa.nickel.util.function.TriFunction;
...
public class ForEachNodeBuilder<FROM extends Iterable<TO>,TO> extends NoSplitNodeBuilder<ForEachNode<FROM,TO>,ForEachNodeBuilder<FROM,TO>> {
    ...
    @Nonnull
    @Override
    public ForEachNode<FROM, TO> build() {
        final ForEachNode<FROM,TO> instance = new ForEachNode<>(key);
        instance.setChildNode(buildChildNode());
        if(transformer != null) instance.setTransformer(transformer);
        return instance;
    }
}
