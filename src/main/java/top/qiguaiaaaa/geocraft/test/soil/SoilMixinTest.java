...
package top.qiguaiaaaa.geocraft.test.soil;
 ...
 * @author QGMoe
 */
public final class SoilMixinTest extends GeoTestItem {

    private HashMap<Block,Class<? extends Block>> injectionClsMap;

    public SoilMixinTest(){
        this.id = new ResourceLocation(GeoCraft.MODID,"soil_mixin_test");
    }
    ...
    @Nonnull
    @Override
    public synchronized EnumActionResult test(@Nonnull final World world, @Nonnull final BlockPos pos, @Nullable final ICommandSender sender) {
        refreshMap();
        boolean success = true;
        for(final @Nonnull Map.Entry<Block,Class<? extends Block>> entry: injectionClsMap.entrySet())
            if(!isExpectedClass(entry.getKey(),entry.getValue(),sender)) success = false;
        injectionClsMap = null;
        return success?EnumActionResult.SUCCESS:EnumActionResult.FAIL;
    }

    public static boolean isExpectedClass(final @Nonnull Block block,final @Nonnull Class<? extends Block> expected,final @Nullable ICommandSender sender){
        if(expected == block.getClass()){
            if(sender != null) sender.sendMessage(translation("geocraft.geotest.soil_mixin_test.success")
                    .arg(Shows.block(block))
                    .arg(expected.getName())
                    .color(TextFormatting.GREEN)
                    .done());
            return true;
        } else{
            if(sender != null) sender.sendMessage(translation("geocraft.geotest.soil_mixin_test.failed")
                    .arg(Shows.block(block))
                    .arg(block.getClass().getName(),expected.getName())
